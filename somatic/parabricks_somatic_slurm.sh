...
pbrun somatic \
	...
	--in-tumor-fq ${FASTQDATA}/${TUMOR}_R1.fastq.gz ${FASTQDATA}/${TUMOR}_R2.fastq.gz "@RG\tID:sm_tumor_rg1\tLB:lib1\tPL:bar\tSM:sm_tumor\tPU:sm_tumor_rg1" \
	--out-tumor-bam ${BAMDATA}/${TUMOR}.bam  \
	--out-tumor-recal-file ${BAMDATA}/${TUMOR}.recal.txt \
	--in-normal-fq ${FASTQDATA}/${NORMAL}_R1.fastq.gz ${FASTQDATA}/${NORMAL}_R2.fastq.gz "@RG\tID:sm_normal_rg1\tLB:lib1\tPL:bar\tSM:sm_normal\tPU:sm_normal_rg1" \
	--out-normal-bam ${BAMDATA}/${NORMAL}.bam \
	--out-normal-recal-file ${BAMDATA}/${NORMAL}.recal.txt \
	--out-vcf  ${TUMOR}_m2.vcf
